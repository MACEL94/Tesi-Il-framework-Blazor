...
\begin{enumerate}
	\item Il service worker, ossia il processo che  che si occupa di aggiornare la posizione della pallina su tutti i client connessi, deve eseguire lato server ed inviare il segnale di aggiornamento a tutti i client connessi;
	\item Il calcolo delle differenze da applicare a ciascun DOM di ciascun cliente, per quanto avvenga continuamente, pu\'o avvenire direttamente lato server perch\'e l'applicazione \'e molto semplice e anche un server free come quello che ho a disposizione riesce a far giocare senza problemi due persone con diversi spettatori.
	\item Essendo la gestione degli eventi server side, la latenza con cui 
